<?php
namespace Koshkil\Framework\DB\Drivers;
use Koshkil\Framework\Core\Exceptions\DB\QueryError;
...
class MysqliDriver {
	...
	public function getRow($sql) {
		...
		if ($this->handler->error) {
			throw new QueryError($this->handler->error);
		}
		return $rec;
	}
	...
	public function getSQL($table,$condition="",$order="",$start=-1,$number=-1) {
		...
		if (is_string($table)) {
			...
			if ($table["debug"]) error_log($having);
		}


		$query="select {$DISTINCT}SQL_CALC_FOUND_ROWS {$fields} from {$tableName} {$where} {$groupBy} {$having} {$order} {$limit}";
		return $query;
	}
